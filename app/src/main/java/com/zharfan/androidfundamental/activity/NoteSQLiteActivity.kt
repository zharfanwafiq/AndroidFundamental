...
import com.zharfan.androidfundamental.adapter.NoteAdapter
import com.zharfan.androidfundamental.data.Note
import com.zharfan.androidfundamental.databinding.ActivityNoteSqliteBinding
import com.zharfan.androidfundamental.db.sqlite.NoteHelper
import com.zharfan.androidfundamental.utils.MappingHelper
...
class NoteSQLiteActivity : AppCompatActivity() {
    ...
    private fun initView() = with(binding) {
        rvNotes.apply {
            layoutManager =
                LinearLayoutManager(this@NoteSQLiteActivity)
    ...
    private fun showData() = with(binding) {
        noteAdapter = NoteAdapter(object : NoteAdapter.OnItemClickCallback {
            override fun onItemClicked(selectedNote: Note?, position: Int?) {
                val intent =
                    Intent(this@NoteSQLiteActivity, NoteAddUpdateActivity::class.java).apply {
            ...
            if (result.data != null) {
                when (result.resultCode) {
                    ...
                    NoteAddUpdateActivity.RESULT_UPDATE -> {
                        ...
                        showSnackBarMessage("Satu Item Berhasil Di Perbarui")
                    }
    ...
    companion object {
        private const val EXTRA_STATE = "com.zharfan.androidfundamental.activity.EXTRA_STATE"
    }
}